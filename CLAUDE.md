...
All source lives in `CockroachPet/`. Seven Swift files, each with a clear responsibility:
...
3. Each tick: updates every cockroach's AI state, then refreshes each cockroach's `CockroachWindow`
4. Handles spawning, killing, persistence (save/restore via UserDefaults)
...
- **`CockroachPetApp`** — App entry point. Sets up `MenuBarExtra` with Summon/Kill All/Settings/Quit. Contains the SwiftUI Settings view. Menu icon updates to show moon emoji during night mode.
...
double-click → dying → dead (+ spawn baby cockroaches)
```
...
- **Persistence via UserDefaults**: Cockroach state (position, state, birth time for babies) is saved on quit and restored on launch. No Core Data or files.
